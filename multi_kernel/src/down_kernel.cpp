...
static void lc_mgvf(INTERFACE_WIDTH *result, INTERFACE_WIDTH *imgvf, INTERFACE_WIDTH *I)
{
	int cols = GRID_COLS;
	int rows = GRID_ROWS;
...
#pragma HLS unroll
			...
			int is_bottom = (i >= GRID_COLS / PARA_FACTOR * (GRID_ROWS - 1));
...
__kernel void down_kernel(INTERFACE_WIDTH *result, INTERFACE_WIDTH *imgvf, INTERFACE_WIDTH *I)
{
    ...
    for(i=0; i<ITERATION/2; i++){
        lc_mgvf(result, imgvf - GRID_COLS/WIDTH_FACTOR, I);
        lc_mgvf(imgvf, result - GRID_COLS/WIDTH_FACTOR, I);
    }
