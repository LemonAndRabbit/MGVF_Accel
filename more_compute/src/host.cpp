...
auto constexpr part_size = (PART_ROWS) * GRID_COLS;
auto constexpr I_size = (PART_ROWS-1) * GRID_COLS;
...
////////////////////RESET FUNCTION//////////////////////////////////
int reset(float* up_MGVF, float* up_I, float* down_MGVF, float* down_I, float* up_results, float* down_results, std::vector<float> check_results) {
    ...
    float temp_MGVF[test_size], temp_I[test_size];
    ...
    for(size_t i=0; i<part_size; i++){
        ...
        down_MGVF[i] = temp_MGVF[i+test_size-part_size];
    }

    for(size_t i=0; i<I_size; i++){
        up_I[i] = temp_I[i];
        down_I[i] = temp_I[i+test_size-I_size];
    }
...
int main(int argc, char** argv) {
    ...
    // I/O Data Vectors
    std::vector<float, aligned_allocator<float> > up_MGVF(part_size);
    std::vector<float, aligned_allocator<float> > up_I(I_size);
    std::vector<float, aligned_allocator<float> > down_MGVF(part_size);
    std::vector<float, aligned_allocator<float> > down_I(I_size);
    std::vector<float, aligned_allocator<float> > up_results(I_size);
    std::vector<float, aligned_allocator<float> > down_results(I_size);
    std::vector<float> check_results(test_size);
    ...
    // Feed input
    //reset(up_MGVF.data(), up_I.data(), down_MGVF.data(), down_I.data(), up_results.data(), down_results.data(), check_results);
    ...
    OCL_CHECK(err, cl::Buffer buffer_up_MGVF(context, CL_MEM_USE_HOST_PTR | CL_MEM_EXT_PTR_XILINX | CL_MEM_READ_WRITE, part_size*sizeof(float), &ptr_up_MGVF,
                                         &err));
    OCL_CHECK(err, cl::Buffer buffer_up_I(context, CL_MEM_USE_HOST_PTR | CL_MEM_EXT_PTR_XILINX | CL_MEM_READ_ONLY, I_size*sizeof(float), &ptr_up_I,
                                         &err));
    OCL_CHECK(err, cl::Buffer buffer_down_MGVF(context, CL_MEM_USE_HOST_PTR | CL_MEM_EXT_PTR_XILINX | CL_MEM_READ_WRITE, part_size*sizeof(float), &ptr_down_MGVF,
                                         &err));
    OCL_CHECK(err, cl::Buffer buffer_down_I(context, CL_MEM_USE_HOST_PTR | CL_MEM_EXT_PTR_XILINX | CL_MEM_READ_ONLY, I_size*sizeof(float), &ptr_down_I,
                                         &err));                                     
                                         
    OCL_CHECK(err, cl::Buffer buffer_up_results(context, CL_MEM_USE_HOST_PTR | CL_MEM_EXT_PTR_XILINX | CL_MEM_READ_WRITE, I_size*sizeof(float), &ptr_up_results,
                                         &err));
    OCL_CHECK(err, cl::Buffer buffer_down_results(context, CL_MEM_USE_HOST_PTR | CL_MEM_EXT_PTR_XILINX | CL_MEM_READ_WRITE, I_size*sizeof(float), &ptr_down_results,
                                         &err));                                     
