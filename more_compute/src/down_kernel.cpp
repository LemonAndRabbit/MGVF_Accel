...
static void lc_mgvf(INTERFACE_WIDTH *result, INTERFACE_WIDTH *imgvf, INTERFACE_WIDTH *I, unsigned int It)
{
	int cols = GRID_COLS;
	int rows = PART_ROWS;
...
#pragma HLS unroll
    ...
    for (i = 0; i < GRID_COLS / PARA_FACTOR * (PART_ROWS-It); i++) {
		int k;
#pragma HLS pipeline II=1
...
#pragma HLS unroll
			...
			int is_bottom = (i >= GRID_COLS / PARA_FACTOR * (PART_ROWS - 1 - It));
...
extern "C" {
__kernel void down_kernel(INTERFACE_WIDTH *result, INTERFACE_WIDTH *imgvf, INTERFACE_WIDTH *I)
{
    ...
    for(i=0; i<ITERATION/2; i++){
        lc_mgvf(result, imgvf + (2*i)*GRID_COLS/WIDTH_FACTOR, I, 2*i+1);
        lc_mgvf(imgvf + (2*i+2)*GRID_COLS/WIDTH_FACTOR, result, I, 2*i+2);
    }
